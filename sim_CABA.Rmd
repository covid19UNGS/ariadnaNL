...
#
fitm <- plot_ajustes(fit,dff,"CABA 0-33", save_plots = TRUE)
...
fitm <- plot_ajustes(fit,dff,"CABA 33-63",save_plots = TRUE)
...
fitm <- plot_ajustes(fit,dff,"CABA 63-93",save_plots = TRUE)
...
fitm <- plot_ajustes(fit,dff,"CABA 93-121",save_plots = TRUE)
...
fitm <- plot_ajustes(fit,dff,"CABA 121-139",save_plots = TRUE)
result <- bind_rows(result, resultados_fit( fitm) %>% mutate(lugar="CABA",periodo_fit="121-139"))
```

## Ajuste despues de los primeros 139-170 días 19/07 hasta 19/08 

```{r fit_lhs139-, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}

df <- dff %>% filter(fecha <=  "2020-08-19")

#
# Ajustando del 63-93 dias
#
fit <- fit_ariadnaNL_simulations(df,139,0,res,pob_CABA,0,100) 
...
#
params <- bind_rows(params, fit %>% 
                      distinct(beta,Horas_en_viaje,Horas_en_trabajo,poblacion,world_width,Proporcion_fallecimiento_hospitalizados,proporcion_hospitalizados,Fallecido_sin_hospitalizacion,ssq) %>% mutate(periodo_fit="139-170", fec_min=fec_min,fec_max=fec_lim,lugar="CABA"))


fitm <- plot_ajustes(fit,dff,"CABA")
result <- bind_rows(result, resultados_fit( fitm) %>% mutate(lugar="CABA",periodo_fit="139-170"))
...
|139-170     | 0.41|           2.74|             2.11|                                   0.092|                      0.19|                         0.005|   0|
...
```{r gen_fited_sim_pfhosp09, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r gen_fited_sim41, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}

nl@experiment <- experiment(expname="world201_Beta41_viaje1_2_trabaj2_75_pfhosp09",
                            outpath=outpath,
                            repetition=1,
                            tickmetrics="true",
                            idsetup="setup",
                            ...
                            runtime=365,
                            metrics=c("count personas", "nro-hospitalizados", "nro-recuperados", "nro-fallecidos","nro-casos-sintomaticos" ),
                            variables = list("beta" = list(values=c(0.41,0.41,0.41,0.41,0.41)),
                                             "Horas-en-viaje" = list(values=c(2   ,2,2  ,1,1)),
                                             "Horas-en-trabajo"=list(values=c(2.11,5,7.5,5,7.5))
                                             ),
                            constants = list("world-width" = 201,
                                             "world-height" = 201,
                                             "infectados-iniciales" = 10,
...
knitr::kable(result %>% select(lugar, periodo_fit, everything()) , digits = 0)
plot_escenarios_CI(fit,dff, "Ajuste CABA",1027,TRUE)
...
res <- read_netlogo_simul("Simulations/world201_Beta41_viaje1_2_trabaj4_75_pfhosp09_distinct.csv",skip = 0) %>% 
                                                mutate(prop_fallecidos = nro_fallecidos / max(count_personas), 
                                                casos = nro_fallecidos+ nro_recuperados, 
                                                fatalidad = nro_fallecidos / casos * 100, poblacion=max(count_personas),
                                                prop_casos=casos/poblacion,prop_camas=capacidad_de_camas/poblacion, 
...
plot_escenarios_CI(fit,dff, "CABA",save_plots=TRUE)
```
