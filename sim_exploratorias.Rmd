...
```{r gen_fit_lhs500, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
res <- read_netlogo_simul("Simulations/world201_Beta30_50_viaje0_3_500_lhs.csv",skip = 0) %>% 
                                                mutate(prop_fallecidos = nro_fallecidos / max(count_personas), 
                                                casos = nro_fallecidos+ nro_recuperados,
                                                fatalidad = nro_fallecidos / casos * 100, poblacion=max(count_personas),
                                                prop_casos=casos/poblacion,prop_camas=capacidad_de_camas/poblacion)

res1 <- read_netlogo_simul("Simulations/world201_Beta30_50_viaje0_3_lhs.csv",skip = 0) %>% 
                                                mutate(prop_fallecidos = nro_fallecidos / max(count_personas), 
                                                casos = nro_fallecidos+ nro_recuperados,
                                                fatalidad = nro_fallecidos / casos * 100, poblacion=max(count_personas),
                                                prop_casos=casos/poblacion,prop_camas=capacidad_de_camas/poblacion,
...
# Guarda parametros
#
params <- fit %>% distinct(beta,Horas_en_viaje,poblacion,world_width,periodo_pre_hospitalizacion,Proporcion_fallecimiento_hospitalizados,sumsqr) %>% mutate(periodo_fit="0-33", fec_min=fec_min,fec_max=fec_lim)
...
#
fit <- fit_ariadnaNL_simulations(df,33,50,res,pob_CABA,3,2) 
...
# Guarda parametros
#
params <- bind_rows(params, fit %>% distinct(beta,Horas_en_viaje,poblacion,world_width,periodo_pre_hospitalizacion,Proporcion_fallecimiento_hospitalizados,sumsqr) %>% mutate(periodo_fit="33-63", fec_min=fec_min,fec_max=fec_lim))
...
#
fit <- fit_ariadnaNL_simulations(df,63,20,res,pob_CABA,3,2) 

fit$fit %>% distinct(beta,Horas_en_viaje,poblacion,world_width,periodo_pre_hospitalizacion,Proporcion_fallecimiento_hospitalizados,sumsqr)
...
# Guarda parametros
#
params <- bind_rows(params, fit %>% distinct(beta,Horas_en_viaje,poblacion,world_width,periodo_pre_hospitalizacion,Proporcion_fallecimiento_hospitalizados,sumsqr) %>% mutate(periodo_fit="63-93", fec_min=fec_min,fec_max=fec_lim))
...
```{r gen_fited_sim200, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE, eval=TRUE}
...
nl@experiment <- experiment(expname="world201_Beta37_34_viaje13_18",
                            ...
                            constants = list("world-width" = 201,
                                             ...
                                             "proporcion-hospiralizados"=0.30,
                                             "capacidad-de-camas"= 70000
                                             ))
